...
<!-- Generated by: python -m cumrag.aggregator --input results/raw/ --output results/aggregated/
     Filter: dataset=rgb -->
...
- Resamples: 1000 (`cumrag.aggregator.BOOTSTRAP_RESAMPLES`)
- Confidence level: 0.95 (`cumrag.aggregator.CI_LEVEL`)
- Flagging threshold: CI width > 15% of mean (`cumrag.aggregator.CI_WIDTH_THRESHOLD`)
- Minimum runs: 3 (`cumrag.aggregator.MIN_RUNS`)
