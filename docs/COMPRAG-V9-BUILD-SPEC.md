# CompRAG v9 — Build Spec

## For Claude Code

The ground truth is `comprag-thesis-v9.md`. If this spec conflicts with the thesis, the thesis wins. If you're unsure, ask — don't invent.

**v9 changes from v8:** Frontier API comparison fully removed (no frontier generation runs, no frontier.yaml, no frontier reference lines in figures). Datasets scoped to RGB only (NQ and HaluEval dropped). Judge model changed to local Command R 35B Q4_K_M (primary bulk judge) with Claude Sonnet 4.6 API (100-record spot-check for appendix validation). 500 queries per model (100 per subset). Hardware updated to x99 mATX board.
...
```
dataset (RGB — counterfactual, noise_robustness, negative_rejection)
    ↓
...
```jsonc
{
  ...
  "quantization": "Q4_K_M",
  "dataset": "rgb",
  "subset": "counterfactual",         // "counterfactual" | "noise_robustness" | "negative_rejection"
  ...
  // Judge metadata
  "judge_model": "command-r-35b-Q4_K_M",  // or "claude-sonnet-4-6" for validation sample
  
  // Error handling
  "error": null,                      // null on success, error category string on failure
  "error_detail": null                // null on success, diagnostic message on failure
}
```

Aggregated output (per model × quant × subset × pass):
...
│   ├── generate.py           # query execution (local only)
...
~200 lines. Local generation only (no frontier API code).
...
**Critical: prompt construction.** `build_messages` returns a standard `messages` array. This goes directly to llama.cpp's `/v1/chat/completions` which applies the model's chat template. The prompt *content* is identical across all models — only the chat template wrapping differs (handled by llama.cpp).

### `comprag/score.py` — evaluation

~200 lines. Thin wrapper around RAGChecker.
...
def score_ragchecker(query: str, response: str, context: list[str], ground_truth: str,
    ...
    Judge: Command R 35B Q4_K_M running on llama.cpp at judge_url.
    RAGChecker uses OpenAI-compatible endpoint via openai/ prefix + api_base override."""

def score_ragchecker_frontier(query: str, response: str, context: list[str], ground_truth: str) -> dict:
    """Same output schema as score_ragchecker.
    Judge: Claude Sonnet 4.6 via Anthropic SDK.
    Used ONLY for the 500-record validation sample.
...
~250 lines. Reads aggregated JSONL, produces matplotlib figures.

**Required figures:**

1. **CU vs Quantization** — X: quant level (Q3→FP16), Y: CU. One curve per model. Error bars from bootstrap CIs. Capability-degraded points get hollow markers.
...
No frontier reference lines (frontier comparison removed in v9).
...
comprag score      --input results/raw/some_run.jsonl --judge-mode frontier  # for 100-record validation
...
models:
  qwen2.5-14b-instruct:
    ...
    quants:
      ...
      FP16:   "qwen2.5-14b-instruct-fp16.gguf"
...
judge:
  ...
  validation:
    ...
    temperature: 0.0
    max_tokens: 1024
    # RAGChecker adapter: use LiteLLM proxy
    ...
    agreement_threshold_kappa: 0.80  # required for CU; secondary metrics report raw agreement alongside κ
```
...
datasets:
  rgb:
    ...
    subsets: [counterfactual, noise_robustness, negative_rejection]
    queries_per_subset: 100
    pass_policy:
...
Score 500 outputs stratified across the model matrix with both the local judge (Command R 35B Q4_K_M) and the frontier judge (Claude Sonnet 4.6 via LiteLLM proxy). Compute Cohen's kappa per metric and per stratum. Output `judge_agreement.json`. κ ≥ 0.80 on CU validates the local judge. For secondary metrics where marginal distributions are skewed (e.g., hallucination, SK near zero), report raw agreement rates alongside κ. Report agreement patterns regardless.

### `scripts/generate_preregistration.py`

Collect: GGUF SHA256s, RAGChecker version, RGB download date, hypothesis text, predictions, statistical criteria, priority claim search results. Write `PRE_REGISTRATION.md`. This gets committed before any experimental run.
...
"Create `comprag/generate.py` with `build_messages` and `generate_local`. Local generation only — no frontier API code. The `build_messages` function constructs messages arrays from `config/prompts.yaml` — no manual chat template tokens. Create `tests/test_generate.py` to verify message construction for all 3 passes."
...
"Create `comprag/visualize.py` producing the 6 figure types defined in the spec. X-axis: quant level (Q3→FP16). No frontier reference lines. Capability-degraded points get hollow markers."
...
11. **No frontier generation code.** All generation is local via llama.cpp. The only API call in the entire pipeline is Sonnet 4.6 for the 100-record judge validation sample.
